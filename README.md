...
python src/run_experiment.py --data-path data/creditcard.csv --output-dir results --fig-dir figures --artifact-path artifacts/stacking_model.joblib
```
...
### Dashboard Startup Wizard & Demo Mode
If `data/creditcard.csv` or `artifacts/stacking_model.joblib` is missing, the app now shows a startup wizard with exact fetch instructions and an optional **Demo Mode** fallback so the UI remains accessible for presentations and deployment checks.
